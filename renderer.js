...
function unlockedGalleryAssetIds() { const ids=new Set();store.collections.filter(gallery=>!gallery.locked).forEach(gallery=>(gallery.items||[]).forEach(id=>{if(!(gallery.discardedIds||[]).includes(id))ids.add(id);}));return ids; }
...
async function reloadImportedMedia() {
  ...
  store = await window.vision.readStore();
  store.sources ||= []; store.collections ||= []; store.assetMeta ||= {}; store.tagDefinitions ||= []; store.personDefinitions ||= []; store.libraryGroups ||= []; store.collections.forEach(collection=>{collection.defaultTags ||= [];collection.defaultPersons ||= [];collection.sourceIds ||= [];collection.discardedIds ||= [];});
  await collectAssets();
  syncGallerySourceAssets();
  await syncSourceWatchers();
...
function renderSettings() {
  ...
  const discardedCount=store.collections.reduce((count,gallery)=>count+(gallery.discardedIds||[]).length,0);
...
async function permanentlyDeleteDiscardEntry(gallery,assetId) { const asset=allAssets.find(item=>item.id===assetId);if(!asset||!confirmAction(`Permanently delete “${asset.name}” from disk? This cannot be undone.`))return;const removed=await window.vision.permanentDelete(asset.path);if(!removed)return toast('Unable to permanently delete this file');for(const source of store.sources)source.assets=await window.vision.scanFolder(source.path);await collectAssets();store.collections.forEach(item=>{item.items=item.items.filter(id=>id!==assetId);item.discardedIds=(item.discardedIds||[]).filter(id=>id!==assetId);item.groups=(item.groups||[]).map(group=>({...group,assets:group.assets.filter(id=>id!==assetId)})).filter(group=>group.assets.length);});await save();selectedId=null;selectedIds.clear();discardOriginGalleryId=null;render(); }
function renderDiscardPile() {
  const canvas=$('#canvas'),empty=$('#emptyState'),entries=discardEntries();empty.classList.add('hidden');canvas.className='discard-pile';canvas.classList.remove('hidden');
  canvas.innerHTML=entries.length?entries.map(({gallery,asset})=>`<article class="discard-card" data-discard-asset="${asset.id}" data-discard-gallery="${gallery.id}"><div class="discard-preview">${asset.type==='image'?`<img src="${fileURL(asset.path)}">`:'<video src="${fileURL(asset.path)}"></video>'}</div><div class="discard-meta"><b>${escapeHTML(asset.name)}</b><small>Gallery: ${escapeHTML(gallery.name)}</small><div><button data-discard-restore>Restore</button><button data-discard-delete class="context-danger">Permanent Delete</button></div></div></article>`).join(''):'<div class="canvas-empty">Discard Pile is empty.</div>';
