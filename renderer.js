...
async function reloadImportedMedia() {
  ...
  store = await window.vision.readStore();
  store.sources ||= []; store.collections ||= []; store.discardedGalleries ||= []; store.assetMeta ||= {}; store.tagDefinitions ||= []; store.personDefinitions ||= []; store.libraryGroups ||= []; store.collections.forEach(collection=>{collection.defaultTags ||= [];collection.defaultPersons ||= [];collection.sourceIds ||= [];collection.manualItemIds ||= [];collection.discardedIds ||= [];});
  await collectAssets();
  const recoveredSourceLinks=inferLegacyGallerySources();
  syncGallerySourceAssets();
  if(recoveredSourceLinks)await save();
...
function syncGallerySourceAssets() {
  ...
  store.collections.forEach(gallery=>{
    gallery.sourceIds ||= [];
    const sourceAssetIds=allAssets.filter(asset=>gallery.sourceIds.includes(asset.sourceId)&&!meta(asset.id).isDuplicate).map(asset=>asset.id);
...
function renderGalleryInspector(gallery) {
  ...
  const sourceCounts=new Map();(gallery.items||[]).filter(id=>!(gallery.discardedIds||[]).includes(id)).forEach(id=>{const asset=allAssets.find(item=>item.id===id),source=asset&&store.sources.find(item=>item.id===asset.sourceId),isManual=(gallery.manualItemIds||[]).includes(id);const name=source&&(gallery.sourceIds||[]).includes(source.id)&&!isManual?source.name:'Khác';sourceCounts.set(name,(sourceCounts.get(name)||0)+1);});$('#galleryMediaSources').innerHTML=sourceCounts.size?[...sourceCounts].map(([name,count])=>`<div class="gallery-source-row"><span>${escapeHTML(name)}</span><b>${count}</b></div>`).join(''):'<span class="muted-small">No media</span>';
...
async function toggleAssetFolder(collectionId,checked) { const collection=store.collections.find(item=>item.id===collectionId),ids=selectedTargetIds();if(!collection||!ids.length)return;ids.forEach(id=>{if(checked){if(!collection.items.includes(id))collection.items.push(id);collection.defaultTags.forEach(tag=>{if(!meta(id).tags.includes(tag))meta(id).tags.push(tag);});(collection.defaultPersons||[]).forEach(tag=>{if(!meta(id).persons.includes(tag))meta(id).persons.push(tag);});}else{collection.items=collection.items.filter(item=>item!==id);collection.manualItemIds=(collection.manualItemIds||[]).filter(item=>item!==id);collection.groups.forEach(group=>group.assets=group.assets.filter(item=>item!==id));removeCollectionTags(id,collection);}});if(checked)markManualItems(collection,ids);await save();renderInspector();renderSidebars(); }
