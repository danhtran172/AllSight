...
async function init() {
  ...
  store.sources ||= []; store.collections ||= []; store.assetMeta ||= {}; store.tagDefinitions ||= []; store.personDefinitions ||= []; store.libraryGroups ||= []; store.collections.forEach(collection=>{collection.defaultTags ||= [];collection.defaultPersons ||= [];collection.sourceIds ||= [];}); store.language ||= 'vi'; store.sourcesCollapsed ||= false; store.zoom ||= 155; applyLanguage(); applyZoom();
  await collectAssets();
...
async function reloadImportedMedia() {
  ...
  store = await window.vision.readStore();
  store.sources ||= []; store.collections ||= []; store.assetMeta ||= {}; store.tagDefinitions ||= []; store.personDefinitions ||= []; store.libraryGroups ||= []; store.collections.forEach(collection=>{collection.defaultTags ||= [];collection.defaultPersons ||= [];collection.sourceIds ||= [];});
  await collectAssets();
  syncGallerySourceAssets();
  await syncSourceWatchers();
...
function openCollectionModal(collection) {
  ...
  const renderSources=()=>{$('#collectionSources').innerHTML=store.sources.length?store.sources.map(source=>`<label class="folder-choice"><input type="checkbox" data-gallery-source="${source.id}" ${selectedSourceIds.has(source.id)?'checked':''}><span>${escapeHTML(source.name)}</span><small>${(source.assets||[]).length} media</small></label>`).join(''):'<span class="muted-small">No media sources yet.</span>';$$('[data-gallery-source]').forEach(input=>input.onchange=()=>input.checked?selectedSourceIds.add(input.dataset.gallerySource):selectedSourceIds.delete(input.dataset.gallerySource));};
  renderSources();
