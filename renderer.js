...
async function init() {
  ...
  store.sources ||= []; store.collections ||= []; store.assetMeta ||= {}; store.tagDefinitions ||= []; store.personDefinitions ||= []; store.libraryGroups ||= []; store.collections.forEach(collection=>{collection.defaultTags ||= [];collection.defaultPersons ||= [];}); store.language ||= 'vi'; store.sourcesCollapsed ||= false; store.zoom ||= 155; applyLanguage(); applyZoom();
  await collectAssets();
...
async function reloadImportedMedia() {
  ...
  store = await window.vision.readStore();
  store.sources ||= []; store.collections ||= []; store.assetMeta ||= {}; store.tagDefinitions ||= []; store.personDefinitions ||= []; store.libraryGroups ||= []; store.collections.forEach(collection=>{collection.defaultTags ||= [];collection.defaultPersons ||= [];});
  await collectAssets();
  await syncSourceWatchers();
...
function visibleAssets() {
  ...
  output = output.filter(asset=>!meta(asset.id).hidden);
  if (collection) output = output.filter(asset => collection.items.includes(asset.id));
...
function applyLanguage() { document.documentElement.lang=store.language; $$('[data-i18n]').forEach(node=>node.textContent=t(node.dataset.i18n)); $('#addSource').textContent='＋'; $('#addSource').title=t('addFolder'); $('#searchInput').placeholder=t('search'); $('#viewOverline').textContent=t('library'); }
...
function renderSidebars() {
  ...
  $('#sourcesList').innerHTML = store.sources.map(source => `<button class="source-item ${currentView === 'source:'+source.id ? 'active':''}" data-source="${source.id}" title="${escapeHTML(source.path)}"><span class="source-icon">▰</span><span class="item-text">${escapeHTML(source.name)}</span><span class="source-remove" data-remove-source="${source.id}" title="Xóa nguồn">×</span></button>`).join('') || '<p class="side-empty">Chưa có thư mục</p>';
  $('.sources-section').classList.toggle('collapsed',!!store.sourcesCollapsed); $('#toggleSources span').textContent=store.sourcesCollapsed?'›':'⌄';
  $('#collectionsList').innerHTML = store.collections.map(collection => {const cover=allAssets.find(asset=>asset.id===(collection.coverId||collection.items[0]));return `<button class="collection-item ${currentView === 'collection:'+collection.id ? 'active':''}" data-collection="${collection.id}">${cover&&cover.type==='image'?`<img class="folder-cover-mini" src="${fileURL(cover.path)}">`:'<span class="gallery-icon">▧</span>'}<span class="item-text">${escapeHTML(collection.name)}</span><span class="collection-count">${collection.items.length}</span></button>`;}).join('') || '<p class="side-empty">Create your first Gallery</p>';
...
function renderLibraryFolders() { const canvas=$('#canvas'),empty=$('#emptyState');empty.classList.add('hidden');canvas.className='folder-gallery';canvas.classList.remove('hidden');canvas.innerHTML=store.collections.map(collection=>{const cover=allAssets.find(asset=>asset.id===(collection.coverId||collection.items[0]));return `<button class="folder-gallery-card" data-library-folder="${collection.id}">${cover&&cover.type==='image'?`<img src="${fileURL(cover.path)}">`:'<span>▧</span>'}<div><b>${escapeHTML(collection.name)}</b><small>${collection.items.length} media</small></div></button>`;}).join('')||'<div class="canvas-empty">Chưa có thư mục cá nhân.</div>';$$('[data-library-folder]').forEach(button=>{button.onclick=()=>{currentView=`collection:${button.dataset.libraryFolder}`;render();};button.oncontextmenu=event=>{event.preventDefault();openFolderContextMenu(event,button.dataset.libraryFolder);};}); }
...
function renderCanvas() {
  ...
  $$('.asset-card').forEach(card => {
    card.addEventListener('click', event => { event.stopPropagation(); const id=card.dataset.id; if(event.ctrlKey||event.metaKey){selectedIds.has(id)?selectedIds.delete(id):selectedIds.add(id);}else {selectedIds=new Set([id]);} selectedId=selectedIds.has(id)?id:[...selectedIds].at(-1)||null; $$('.asset-card').forEach(item=>item.classList.toggle('selected',selectedIds.has(item.dataset.id))); $('#inspector').classList.toggle('hidden',selectedIds.size!==1); updateSelectionUI(); renderInspector(); });
  ...
  $$('.group-shell').forEach(group => {
    ...
    group.addEventListener('contextmenu',event=>{if(event.target.closest('.asset-card'))return;event.preventDefault();openGroupContextMenu(event,group.dataset.group);});
  });
...
function scheduleUngroup(assetId,targetId) { const key=`ungroup:${assetId}:${targetId||'end'}`; if(hoverTimer&&hoverTargetId===key)return; clearHover();hoverTargetId=key;const group=groupOf(assetId);if(!group)return;const source=$(`.asset-card[data-id="${assetId}"]`);source?.classList.add('ungroup-target');source?.insertAdjacentHTML('beforeend','<span class="hold-progress"></span>');const deadline=performance.now()+3000;document.body.classList.add('drag-holding');hoverTimer=setTimeout(()=>{if(performance.now()>=deadline)removeFromGroup(assetId,targetId);clearHover();},Math.max(0,deadline-performance.now())); }
async function removeFromGroup(assetId,targetId) { const group=groupOf(assetId);if(!group)return;group.assets=group.assets.filter(id=>id!==assetId);if(!group.assets.length){const groups=activeGroups(),index=groups.indexOf(group);groups.splice(index,1);}if(targetId)meta(assetId).order=(meta(targetId).order||Date.now())+.5;else meta(assetId).order=Math.min(...visibleAssets().map(asset=>meta(asset.id).order||asset.modified),Date.now())-1;await save();toast('Đã tách ảnh khỏi nhóm');render(); }
...
function openCollectionModal(collection) {
  ...
  $('#saveCollection').addEventListener('click',async()=>{const name=$('#collectionName').value.trim();if(!name)return; const details={name,note:$('#collectionNote').value.trim(),defaultTags:$('#collectionTags').value.split(',').map(x=>x.trim()).filter(Boolean),defaultPersons:$('#collectionPersons').value.split(',').map(x=>x.trim()).filter(Boolean),color:picked}; if(collection) Object.assign(collection,details); else {const fresh={id:uid(),items:[],groups:[],...details};store.collections.push(fresh);currentView=`collection:${fresh.id}`;} await save(); closeModal();render();});
}
...
function openContextMenu(event,assetId) {
  ...
  if(ids.length>=2) {
    menu.innerHTML='<button data-context-bulk-group>Create group</button><button data-context-bulk-hide>Hide image</button><button data-context-bulk-gallery>Add to Gallery</button>';
    menu.querySelector('[data-context-bulk-group]').onclick=()=>{closeContextMenu();bulkCreateGroup();};
...
function showUnlock() { document.body.innerHTML=`<div class="unlock-screen"><div class="unlock-card"><div class="brand"><span class="brand-mark">A</span><span>allsight</span></div><h1>Không gian riêng tư</h1><p>Nhập mật khẩu để mở thư viện của bạn.</p><input type="password" id="unlockInput" autofocus placeholder="Mật khẩu"><button id="unlockButton" class="primary-button">Mở khóa</button><small id="unlockError"></small></div></div>`; const go=async()=>{if(await hash($('#unlockInput').value)===store.passwordHash){sessionStorage.setItem('master-vision-unlocked', store.passwordHash);location.reload()}else{$('#unlockError').textContent='Mật khẩu chưa chính xác';$('#unlockInput').select();}};$('#unlockButton').onclick=go;$('#unlockInput').onkeydown=e=>{if(e.key==='Enter')go();}; }
